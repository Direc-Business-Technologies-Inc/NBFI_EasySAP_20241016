    ...
    class ITmaintenance
    {
        ...
        public string SelValue(string ITRqrycode, string strTransType, [Optional] string strBPCode, [Optional] string strAddress, [Optional] string strTxtbox)
        {
            ...
            if (ITRqrycode == "series1" && strTransType != "")
            {
                strSelectedQry = $"SELECT " +
                                $" (Select SeriesName from NNM1 where SeriesName = T1.U_ITSeries and ObjectCode = '67' ) [Value] " +
        ...
        public string GetCompany(string BPcode, string ItemCode)
        {
            ...
            if (Hana.Get( strSelQry).Rows.Count > 0)
            {
                ...
                if (CompanyCode != "")
                {
                    CompanyCode = Hana.Get( $"select Name from [@CMP_INFO] where Code = '{CompanyCode}'").Rows[0]["Name"].ToString();
                }
            }
        ...
        public string GetUOM(string sItemCode)
        {
            ...
            sInvntoryUom = Hana.Get( $"select InvntryUom from OITM where ItemCode = '{sItemCode}'").Rows[0]["InvntryUom"].ToString();
