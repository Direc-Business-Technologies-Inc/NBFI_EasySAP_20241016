...
{
    public class PurchaseOrderModel : IPurchaseOrderModel
    {
        ...
        public bool ActivateService(Func<string, string> returnMessage, string request, string docEntry, StringBuilder json, StringBuilder jsonH, bool isUserGoApproval, bool isApproved = false, string bstat = "Open")
        {
            var isPosted = false;
            ...
            if ((request == "Add") || (request == "Update" && isApproved) || (request == "Update" && bstat == "Draft-Rejected"))
            {
               
             
                isPosted = SAPHana.SL_PostingWithApproval("PurchaseOrders", json, out string sMsg, docEntry, isApproved);
                message = sMsg != "" ? sMsg : "";
                returnMessage(sMsg);
            }
            else
            {
                string url = $"PurchaseOrders({docEntry})";
                //isPosted = SAPHana.SL_AJAX((value) => returnvalue = value, url, "PATCH", json.ToString(), "DocEntry");
                isPosted = serviceLayer.ServiceLayer_Posting(json, "PATCH", url, "DocEntry", out err, out svalue);
        ...
        {
            bool isSuccess = true;
            string url = $"PurchaseOrders({docEntry})/Cancel";
        ...
        public DataTable GetUDF()
        {
            ...
            return DataRepository.GetData(SP.UDF("OPOR"));
        }
        ...
        {
            return DataRepository.GetData(_Repo.PurchaseOrderExistingDocs());
        }
        ...
        public List<string> GetGL(string TransType)
        {
            var dt = DataRepository.GetData(_Repo.GetGLAccount(TransType));
