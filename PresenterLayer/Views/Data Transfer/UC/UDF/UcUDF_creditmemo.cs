...
{
    public partial class UcUDF_creditmemo: UserControl
    {
        SAPHanaAccess hana { get; set; }
        DataHelper helper { get; set; }
        public UcUDF_creditmemo()
        {
            InitializeComponent();
        ...
        private void pbBpCode_Click(object sender, EventArgs e)
        {
            var fS = new frmSearch2();
            fS.oSearchMode = "OCRD";
            frmSearch2.Param1 = "C";
            frmSearch2._title = "List of Business Partners";
            fS.ShowDialog();

            if (fS.oCode != null)
            {
                txtCardCode.Text = fS.oCode;
                frmDT_UDF.CardCode = fS.oCode;
            }
        }
    }
}
