...
{
    public partial class FrmSalesOrderItemList : MetroForm
    {
        private SalesOrderItemsController soic = new SalesOrderItemsController();
        ...
        private FrmSalesOrder SalesOrder;
        private static int defaultColumn, _rowIndex = 0;
        ...
        public FrmSalesOrderItemList([Optional] FrmSalesOrder SalesOrder, [Optional] MainForm frmMain)
        {
        ...
        private void FrmSalesOrderItemList_Load(object sender, EventArgs e)
        {
                ...
                foreach (DataGridViewRow row in gvSO.Rows)
                {
                    try
                    {
                        if (row.Cells[defaultColumn].Value.ToString().ToUpper().StartsWith(txtSearch.Text.ToUpper()))
                        {
                            row.Selected = true;
                            _rowIndex = row.Index;
                            gvSO.FirstDisplayedScrollingRowIndex = _rowIndex;
                            break;
                        }
                        else
                        {
        ...
        private void txtSearch_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            ...
            //Part of Change Request 01/13/2020
        ...
        private void gvSO_Click(object sender, EventArgs e)
        {
            SelectItemList(false);
        }
        ...
        private void gvSelectedItem_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (ColumnName == "Quantity")
            {
                ...
                gvSelectedItem.CurrentRow.Cells["Ordered Quantity"].Value = dOrigQty; //Requested to remove logic 07212021 //SalesOrderItemsController.GetCartonQty(sItemCode, dOrigQty);
            }
        }
                                        ...
                                        , string strDept, string strSubDept, string SubColor, string strSubSize, string frmgvSO, string strExludeItems)
        {
            ...
            DateTime dtPostDate = SalesOrder.dtPostingDate.Value;
            ...
            query = "SELECT *, MT1.GrossPriceBase * (1 +'.'+ Cast(MT1.TaxRate as INT)) [GrossPrice]  FROM ( " +
                    ...
                    ", T0.CodeBars [BarCode]" +
                    ", T0.ItemName" +
                    ", (select distinct T3.U_SKU " +
                    " FROM OCPN T1 " +
                    $" INNER JOIN CPN1 T2 ON T1.CpnNo = T2.CpnNo AND T2.BpCode = '{SalesOrdersHeaderModel.oBPCode}' " +
                    " INNER JOIN CPN2 T3 ON T2.CpnNo = T3.CpnNo AND T3.ItemCode = T0.ItemCode " +
                    " WHERE " +
                    $" T1.Status = 'O' AND ISNULL('{dtPostDate.ToString("yyyy/MM/dd")}', dateadd(dd, 180, GetDate())) between T1.StartDate and ISNULL(T1.FinishDate, dateadd(dd, 180, GetDate()))) [SKU]" +
                    ", ISNULL((select OnHand - IsCommited + OnOrder from OITW where ItemCode = T1.ItemCode and WhsCode = '" + SalesOrdersHeaderModel.oWhsCode + "'), 0) [Available] " +
                    ", '" + SalesOrdersHeaderModel.oWhsCode + "' [Warehouse]";
            
            int iPriceList = int.Parse(helper.ReadDataRow(hana.Get($"SELECT ISNULL(ListNum,1) FROM OCRD WHERE CardCode = '{SalesOrdersHeaderModel.oBPCode}'"), 0, "", 0));
            ...
            if (SalesOrdersHeaderModel.oBPCode != "")
            {
                query += $" , CASE WHEN ISNULL((SELECT z.Price From OSPP z Where z.ItemCode = T0.ItemCode and z.CardCode = '{SalesOrdersHeaderModel.oBPCode}'),0) = 0) " +
                        $" THEN ISNULL((select Price from ITM1 where ItemCode = T0.ItemCode and PriceList = '2'), 0) " +
                        $" ELSE ISNULL((SELECT z.Price From OSPP z Where z.ItemCode = T0.ItemCode and z.CardCode = '{SalesOrdersHeaderModel.oBPCode}'),0) [EffectivePrice] ";

                query += $" , CASE WHEN ISNULL((SELECT z.Price From OSPP z Where z.ItemCode = T0.ItemCode and z.CardCode = '{SalesOrdersHeaderModel.oBPCode}'),0) = 0) " +
                        $" THEN ISNULL((select Price from ITM1 where ItemCode = T0.ItemCode and PriceList = '{iPriceList}'), 0) " +
                        $" ELSE ISNULL((SELECT z.Price From OSPP z Where z.ItemCode = T0.ItemCode and z.CardCode = '{SalesOrdersHeaderModel.oBPCode}'),0) [GrossPriceBase] ";
            }
            else
            {
            ...
            query += $" , ISNULL((select Price from ITM1 where ItemCode = T1.ItemCode and PriceList = '{iPriceList}'), 0) [UnitPrice] " +
                    $" , '{SalesOrdersHeaderModel.oTaxGroup}' [Tax Code]" +
                    $", (select Rate from OVTG Where Code = '" + SalesOrdersHeaderModel.oTaxGroup + "' ) [TaxRate] " +
                    " , '0' [Tax Amount] " +
                    " , '0' [Line Total] ";

            if (SalesOrdersHeaderModel.oBPCode != "")
            {
                query += $" , CASE WHEN ISNULL((SELECT Discount from OSPP where CardCode = '{SalesOrdersHeaderModel.oBPCode}' and ItemCode = T0.ItemCode),0) = 0) " +
                        $" THEN ISNULL((select 100 - (Factor*100) [Discount] from OPLN where ListNum = '{iPriceList}'), 0) " +
                        $" ELSE ISNULL((SELECT Discount from OSPP where CardCode = '{SalesOrdersHeaderModel.oBPCode}' and ItemCode = T0.ItemCode),0) [Discount] ";
            }
            else
            {
                query += ", '0' [Discount]";
        ...
        private Boolean AddItem()
        {
            ...
            try
            {
                ...
                {
                    var getItemDetails = helper.ReadDataRow(hana.Get(SP.AW_GetItemDetails), 1, "", 0);
                    var dt = hana.Get(string.Format(getItemDetails, row.Cells[4].Value.ToString()));
                    ...
                    if ((row.Cells["Quantity"].Value != null))
                    {
                        ...
                        if (SOm.SelValue("AllowDupItems", SalesOrdersHeaderModel.oDocType) != "Y" && SalesOrderItemsModel.SalesOrderItems.Where(x => x.ItemCode == strItemCode).ToList().Count() >= 1)
                        {
                            AllowDupItem = false;
                        }
                        ...
                        if (AllowDupItem)
                        {
                            if (SalesOrderItemsModel.SalesOrderItems.Count > 0)
                            {
                                if (SalesOrderItemsModel.SalesOrderItems.Select(x => x.Linenum).Max() != 0)
                                {
                                    index = SalesOrderItemsModel.SalesOrderItems.Select(x => x.Linenum).Max() + 1;
                                }
                            }
                            ...
                            {
                                Linenum = iIndex,
                                ObjType = FrmSalesOrder.objType, //ObjType
                                ...
                                EffectivePrice = Convert.ToDouble(DECLARE.Replace(row, "Effective Price", "0")),
                                ...
                                GrossTotal = Convert.ToDouble(DECLARE.Replace(row, "Gross Price", "0")),
                                //DiscountAmount = Convert.ToDouble(DECLARE.Replace(row, "Discount Amt", "0")),  
                                SKU = row.Cells[19].Value.ToString(), //SKU
                                OrderedQty = Convert.ToDouble(DECLARE.Replace(row, "Ordered Quantity", "0"))
                            });
                        }
                        else
                        {
        ...
        private void CalculateTotalPages()
        {
            ...
            dt = hana.Get(query);
            Int64 rowCount = Convert.ToInt64(helper.ReadDataRow(dt, "Count", "",0) );
